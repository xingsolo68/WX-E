...
export class DiscountFactory {
    static async create(overridedFields = {}) {
        ...
        const defaultFields = {
            ...
            maxValue: faker.datatype.number({ min: 100, max: 500 }),
            code: faker.random.alphaNumeric(8),
            startDate: faker.date.soon(),
            endDate: faker.date.future(),
            maxUses: faker.datatype.number({ min: 1, max: 10 }),
            useCount: 0,
            userUses: [],
            maxUsesPerUser: faker.datatype.number({ min: 1, max: 5 }),
            minOrderValue: faker.datatype.number({ min: 50, max: 200 }),
            isActive: true,
            appliesTo: 'all',
            shopId: shop.id,
        }
