...
const awsService = {
    init: async () => {
        try {
            ...
            logger.info('[AWS] AWS service initialized')
        } catch (error) {
            logger.error('[AWS] Error during AWS service initialization')
            throw error
        }
    },
    ...
    uploadFile: async (file, extension, bucketPath = '') => {
        ...
        if (!file.uuid)
            console.log(
                `[AWS] File ${file.filename} has no uuid, creating uuid`
            )
        ...
        try {
            ...
            logger.info(
                `[AWS] File uploaded successfully at ${response.Location}`
            )

            return _uuid
        } catch (error) {
            logger.error('[AWS] Error during file upload')
            throw error
        } finally {
            logger.info(
    ...
    uploadFileAsBase64: async (
        ...
        if (!preferredUUID)
            logger.verbose(`[AWS] File  has no uuid, creating uuid`)

        logger.verbose(`[AWS] Creating base64 file uuid`)
        const _uuid = preferredUUID || uuidv4()
        logger.verbose(`[AWS] Assigned uuid ${_uuid} to base64 file`)
    ...
    deleteFile: async (fileUUID, extension, bucketPath = '') => {
        try {
            ...
            logger.verbose(`[AWS] File ${fileUUID} deleted succefully.`)

            return
        } catch (error) {
            logger.verbose('[AWS] Error during file deletion')
            throw error
        }
    },
}
